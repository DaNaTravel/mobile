...
const HistoryItem = ({item, type}) => {
  ...
  const CreateListImg = () => {
    item?.itinerary?.people === undefined ? setDataImg(item?.routes
      .map(route => route.photos)
      .filter(photo => photo !== null)) : null
  }
  useEffect(() => {
    CreateListImg();
  }, [])
    ...
    <View style={styles.viewParent}>
      <View style={styles.viewContainer1}>
        <FlatList
          ...
          renderItem={({item, index}) => (
            <Image style={styles.viewImg} source={item ? {
      ...
      <View style={styles.viewContainer2}>
        <View style={styles.viewDate}>
          ...
          <View style={styles.viewDetailDate}>
            <Text style={styles.textDate}>
              {item?.itinerary?.days !== undefined ? item?.itinerary?.days : item?.days} Days
            </Text>
        ...
        <View style={styles.viewPeople}>
          ...
          <View style={styles.viewDetailDate}>
            <Text style={styles.textDate}>
              {item?.itinerary?.people !== undefined ? item?.itinerary?.people : item?.people} peoples
            </Text>
      ...
      <View style={styles.viewContainer3}>
        <View style={styles.viewPrice}>
          ...
          <Text style={styles.textPrice}>
            ${item?.itinerary?.cost!== undefined ? item?.itinerary?.cost : item?.cost}.00
          </Text>
        </View>
        <TouchableOpacity
