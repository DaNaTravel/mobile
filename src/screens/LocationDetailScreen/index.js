...
const Header = ({navigation, data, booked, handleBook, setAlert}) => {
    ...
    <>
      ...
      <View style={styles.viewMainContent}>
        <Carousel
          ...
          layoutCardOffset={9}
          ref={isCarousel}
          data={data?.photos !== null ? data?.photos : dataImage}
          ...
          key={item => item._id}
        />
        {<PaginationCarousel data={data?.photos} index={index} />}
        <View style={styles.viewName}>
          <View style={styles.viewNamePrice}>
            <Text style={styles.name} numberOfLines={1}>
              {data?.name ? data?.name : 'BaNa Hills'}
            </Text>
            <Text style={styles.price}>${data?.cost ? data?.cost : '20'}</Text>
          </View>
...
const LocationDetail = ({route}) => {
  ...
  useEffect(() => {
    SearchByID(item?.description?._id, setData);
    setDataRelated(data?.relatedLocations);
    console.log('load');
  }, [item]);
  return (
